...
def mark_cross_line( frame, arg_x, arg_y, arg_color=(0,0,255), arg_linewidth=2):
    cv2.line(frame , (int(arg_x) - 15 , int(arg_y)) , (int(arg_x + 15) , int(arg_y)) , arg_color , arg_linewidth)
    cv2.line(frame , (int(arg_x) , int(arg_y - 15)) , (int(arg_x) , int(arg_y + 15)) , arg_color , arg_linewidth)
    return frame
...
def binarialization(arg_frame, arg_binaryMethod, arg_thresholdValue= 100, arg_inverse= False):
    ...
    if arg_inverse:
        binaryIndex= cv2.THRESH_BINARY
    else:
        binaryIndex= cv2.THRESH_BINARY_INV 

    if arg_binaryMethod== 0:
	ret, thresholdedImg= cv2.threshold(blur.copy() , arg_thresholdValue, 255 , binaryIndex)
    elif arg_binaryMethod == 1:
	ret,thresholdedImg = cv2.threshold(blur.copy(),0 ,255 ,binaryIndex+cv2.THRESH_OTSU)
    elif arg_binaryMethod== 2:
	thresholdedImg = cv2.adaptiveThreshold(blur.copy(),255,cv2.ADAPTIVE_THRESH_GAUSSIAN_C,binaryIndex,5,0)
...
def HoughLine(arg_img):
    ...
    if lines is not None:
        ...
        cv2.putText(img, 'No line is founded...',(30,15),cv2.FONT_HERSHEY_SIMPLEX, 2, (0,0,255))
    return img

def pca_contour(arg_ctr):
